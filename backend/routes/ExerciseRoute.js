...
// Save a new workout for the user
router.post("/save", async (req, res) => {
  const { userId, nameWorkout, duration, repetition, date } = req.body;
  if (!userId || !nameWorkout || !duration || !repetition) {
  ...
  try {
    ...
    user.workouts.push({ nameWorkout, duration, repetition, date: date ? new Date(date) : new Date() });
    await user.save();
